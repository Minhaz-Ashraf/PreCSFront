  ...
  <span className=" flex  md:flex-row flex-col-reverse sm:flex-row relative md:mx-16 mx-6 mb-0 md:h-[620px] h-[630px]">
    ...
    <img src={heroCircles} alt="ellipse"  className="absolute top-80 right-0 w-36 popimg "/>
      ...
      <span className="flex flex-row items-center font-DMsans mt-9 gap-9">
        <span onClick={openPopup} className="border popimg border-primary text-primary px-6 py-2 cursor-pointer rounded-lg">Sign up</span>
        <span className="px-6 py-2 bg-primary text-white cursor-pointer rounded-lg hidden">Log in</span>
...
{/* //why choose us */}
      <span>
        <span className="flex justify-center items-center mt-9">
  ...
  {chooseData.map((i, index) => (
    <React.Fragment key={index}>
      <div className="flex flex-col justify-center items-center md:w-[30%] sm:w-[30%] w-[70%] mt-14">
        ...
        <p className="font text-[13px] text-black text-center text-wrap w-2/3">
          {i.text}
        </p>
      </div>
      {index !== 5 && (
        ...
        <span className="grid grid-cols-1 md:grid-cols-4 ss:grid-cols-2 sm:grid-cols-2 gap-8  px-16 mt-12 font-DMsans">
          {cardData.map((m, index) => (
            <span className="rounded-2xl py-5 pb-9  card ">
              ...
              <span className="flex items-center justify-center">
                <p className="text-[14px] pt-5 px-5 font-medium font-DMsans text-start ">
                  {m.text}
                </p>
              </span>
            </span>
